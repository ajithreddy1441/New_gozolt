      ...
      if (res.ok && data.success) {
        ...
        const commonEmailData = {
          to_email: driverDetails.email,
            ...
            booking_reference: data.reservation_no || data.data?.booking_id || 'N/A',
            supplier_reference: data.supplier_reference || 'N/A',
            car_model: carDetail?.model_type || carDetail?.model || 'Car',
            car_category: carDetail?.category || '',
            car_image_url: carDetail?.imageUrl || carDetail?.car_image || '/car-img.png',
            car_seats: carDetail?.seats || 'N/A',
            car_transmission: carDetail?.transmission_type || carDetail?.transmission || 'N/A',
            car_bags: carDetail?.bags || 0,
            car_ac: carDetail?.air_conditioning ? 'Air Conditioning' : 'No AC',
            price_per_day: basePrice.toFixed(2),
            total_price: calculateTotal().toFixed(2),
            pickup_location: searchParams?.location || carDetail?.location?.name || 'Unknown location',
            dropoff_location: searchParams?.location || carDetail?.location?.name || 'Unknown location',
            pickup_date: searchParams?.pickup_date ? formatDateForAPI(searchParams.pickup_date) : '',
            pickup_time: searchParams?.pickup_time || '',
            dropoff_date: searchParams?.return_date ? formatDateForAPI(searchParams.return_date) : '',
            dropoff_time: searchParams?.return_time || '',
